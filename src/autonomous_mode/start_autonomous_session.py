...
def _select_next_ball(robot: Robot, balls_in_robot: int, state: ArenaState):
    ...
    next_ball = robot.get_nearest_vip_ball(state.balls) if vips_on_field and (balls_in_robot == BALLS_PER_DELIVERY - 1 or balls_in_robot == state.estimated_ball_count - 1) else robot.get_nearest_non_vip_ball(state.balls)
